...
namespace MavLink
{
    ...
    public static class MavLinkSerializer
    {
        ...
        public static Dictionary<int, MavPacketInfo> Lookup = new Dictionary<int, MavPacketInfo>
        {
			...
			{141, new MavPacketInfo(Deserialize_ALTITUDE, 148)},
			...
			{245, new MavPacketInfo(Deserialize_VTOL_STATE, 83)},
		...
		internal static MavlinkMessage Deserialize_ALTITUDE(byte[] bytes, int offset)
		{
			...
			{
				altitude_monotonic = bitconverter.ToSingle(bytes, offset + 0),
				altitude_amsl = bitconverter.ToSingle(bytes, offset + 4),
				altitude_local = bitconverter.ToSingle(bytes, offset + 8),
				altitude_relative = bitconverter.ToSingle(bytes, offset + 12),
				altitude_terrain = bitconverter.ToSingle(bytes, offset + 16),
				bottom_clearance = bitconverter.ToSingle(bytes, offset + 20),
			};
		}
		...
		internal static MavlinkMessage Deserialize_CONTROL_SYSTEM_STATE(byte[] bytes, int offset)
		{
			return new Msg_control_system_state
			{
				...
				pos_variance =  ByteArrayUtil.ToSingle(bytes, offset + 51, 3),
				q =  ByteArrayUtil.ToSingle(bytes, offset + 54, 4),
				roll_rate = bitconverter.ToSingle(bytes, offset + 58),
				pitch_rate = bitconverter.ToSingle(bytes, offset + 62),
				yaw_rate = bitconverter.ToSingle(bytes, offset + 66),
			};
		}
		...
		internal static MavlinkMessage Deserialize_HOME_POSITION(byte[] bytes, int offset)
		{
			return new Msg_home_position
			{
				...
				approach_x = bitconverter.ToSingle(bytes, offset + 28),
				approach_y = bitconverter.ToSingle(bytes, offset + 32),
				approach_z = bitconverter.ToSingle(bytes, offset + 36),
			};
		}
		...
		internal static MavlinkMessage Deserialize_VTOL_STATE(byte[] bytes, int offset)
		{
			return new Msg_vtol_state
			{
		...
		{
			bitconverter.GetBytes(msg.altitude_monotonic, bytes, offset + 0);
			bitconverter.GetBytes(msg.altitude_amsl, bytes, offset + 4);
			bitconverter.GetBytes(msg.altitude_local, bytes, offset + 8);
			bitconverter.GetBytes(msg.altitude_relative, bytes, offset + 12);
			bitconverter.GetBytes(msg.altitude_terrain, bytes, offset + 16);
			bitconverter.GetBytes(msg.bottom_clearance, bytes, offset + 20);
			offset += 24;
			return 141;
		}
		...
		internal static int Serialize_VTOL_STATE(this Msg_vtol_state msg, byte[] bytes, ref int offset)
		{
			bytes[offset + 0] = msg.state;
			offset += 1;
			return 245;
		}
